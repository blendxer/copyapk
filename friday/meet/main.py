...
# save_image_path = 'tex\\tbk.jpg'
# save_thumb = 'tex\\ttn.png'
# changed_background = 'tex\\cbg.jpg'
# changed_thumb = 'tex\ctn.png'
# dev_background = 'tex\defbg.jpg'
# dev_thumb = 'tex\defthum.png'

# current_background_load = r'tex\defbg.jpg'
# current_thumb_load = r'tex\defthum.png'



save_image_path =os.path.join('tex','tbk.jpg')
save_thumb = os.path.join('tex','ttn.png')
changed_background =os.path.join('tex','cbg.jpg')
changed_thumb = os.path.join('tex','ctn.png')
dev_background = os.path.join('tex','defbg.jpg')
dev_thumb = os.path.join('tex','defthum.png')
current_background_load = os.path.join('tex','defbg.jpg')
current_thumb_load = os.path.join('tex','defthum.png')
...
class Meet_Lobby(ScreenManager):
    ...
    ############  change background
    thumb_image = theme_load['current_thumb'] 
    ...
    def change_calender(self):
        ...
        if len(self.today_lessons):
            ...
            _shift = dp(5)
            other_day_button = len(self.other_day_lessons)*button_height+30
            ...
            for k in range(len(self.today_lessons)):
                with self.box_canvas.canvas.before:
                    ...
                    Line(points=(wid+_shift,wid+dp(y_pos)+other_day_button, 
                                wid+_shift,50-2*wid+dp(y_pos)+other_day_button,
                                root_width-wid-_shift,50-2*wid+dp(y_pos)+other_day_button,
                                root_width-wid-_shift,wid+dp(y_pos)+other_day_button),close=True, width=wid,group='aa')
                    y_pos+=50
        ...
        if len(self.other_day_lessons):
            ...
            for k in range(1,len(self.other_day_lessons)+1):
                with self.box_canvas_2.canvas.before:
                    ...
                    Line(points=(wid+_shift,wid+dp(y_pos), 
                                wid+_shift,dp(50-2*wid)+dp(y_pos),
                                root_width-wid-_shift,dp(50-2*wid)+dp(y_pos),
            ...
            for j in self.other_day_lessons:
                ...
                background_normal='',background_color=(0,0,0,0),markup = True,font_size=self.lessons_font_size,color=(self.lessons_text_color))
            ...
            today_lab_2 = Button(text= self.to_arabic('لايوجد دروس أخرى'),size_hint_y=None,height= label_height,
                    ...
                    background_down='',font_size=dp(self.lessons_font_size))
            self.ids['lessons_table'].add_widget(today_lab_2)
        self.ids['lessons_table'].height = dp((label_height*2)+(len(self.today_lessons)+len(self.other_day_lessons))*button_height)
    ...
    def class_del(self):
        ...
        if self.ids['mat_name'].text in lessons:
            ...
            print('new content has been deleted')
            AAA.text_saver()
        else:
            print('the lessons not in lessons dir')
        Ar_text.str = ''
    ...
    def toaster(self, message):
        #toast(message)
        pop = Popup(title=message,title_font='arabic',size_hint_y=None,pos=(0,100),
        title_size=20)
        pop.open()
###########3  Time Setter ############
...
#########     setting method ##########
    ...
    def apply_new_setting(self):
        ...
        theme_load = {'lessons_text_color':self.lessons_text_color ,
                    'lessons_canvas_color':self.lessons_canvas_color,
                    'lessons_font_size':self.lessons_font_size,
                    'meet_link_canvas_color':self.lessons_canvas_color,
                    'current_background': self.ids.image_background_.source,
                        }
        if self.th_image:
        ...
        t = threading.Thread(target=save_theme_2)
        t.start()
...
class AAA():
    def text_saver():
        t = threading.Thread(target=apply_text_saver)
        t.start()
