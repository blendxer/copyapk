        ...
        try:
            ...
            if len(self.today_lessons):
                today_lab = Button(text= self.to_arabic('دروس اليوم'),size_hint_y=None,height= label_height,
                        font_name='arabic',background_normal='',background_down='',background_color=[0,0,0,0],
                        color=(self.lessons_text_color),font_size=dp(self.lessons_font_size))
...
class meet_libApp(MDApp):
    ...
    def on_start(self):
        Clock.schedule_once(self.change_button_color, 7)
    
    def change_button_color(self, *args):
        pass
        # for i in lessons_arange:
        #     self.root.ids[i].background_color = [1,0,0,0]
        #     self.root.ids[i].color = 0.6,0.6,0.6,1

if __name__ == '__main__':
    #Config.set('inputt', 'mouse', 'mouse,disable_multitouch')
    meet_libApp().run()
